...
def get_obs_era5_timeseries(startdate,enddate,timestep,lat_slice,lon_slice,sigma):
    ...
    while currentdate.year <= enddate.year:
        ...
        var = ['V-wind','logp']#var = ['U-wind','logp']
        ds_nature = ds_nature['V-wind'].sel(Lat=lat_slice,Lon=lon_slice,Sigma_Level=sigma)
...
print('average rmse Hybrid', np.average(analysis_rmse[:-1:]))
print('average rmse SPEEDY', np.average(analysis_rmse_speedy[::]))
