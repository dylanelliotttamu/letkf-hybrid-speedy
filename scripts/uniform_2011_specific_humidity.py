...
analysis_file_speedy = '/skydata2/dylanelliott/letkf-hybrid-speedy/DATA/uniform_letkf_anal/40_member_speedy_covar1_5_20110101_20120901/mean.nc'#'/skydata2/dylanelliott/letkf-hybrid-speedy/DATA/uniform_letkf_anal/uniform_analysis_2012_01_to_2012_09.nc'
...
xgrid = 96
ygrid = 48
length = 592#480#240  #1450 ##338 #160#64#177#1400#455
...
plt.colorbar(cf,label='(g/kg)')
ax2.set_title('40 MEMBER COV=1.5\nSPEEDY LETKF Analysis Error\n Low Level Specific Humidity')
...
plt.plot(date_list,analysis_rmse,color='r',label='RMSE New Hybrid') #cov-infl1.3
...
plt.title('40 MEMBER ENSEMBLE COV=1.5\nLETKF Analysis Error\n Low Level Specific Humidity')
...
#plot of spin up 
date_list = [base + timedelta(days=x/4) for x in range(length)]
plt.plot(date_list,analysis_rmse_speedy,color='b',label='RMSE SPEEDY COV=1.5')
plt.axhline(y=np.average(analysis_rmse_speedy[20::]), color='b', linestyle='--',label="Average RMSE SPEEDYCOV=1.5")

plt.plot(date_list,analysis_rmse,color='r',label='RMSE New Hybrid COV=1.5') #cov-infl1.3
plt.axhline(y=np.average(analysis_rmse[20::]), color='r', linestyle='--',label="Average New RMSE Hybrid COV=1.5")
