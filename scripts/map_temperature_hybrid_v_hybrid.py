...
print('the smallest length before adjustment is:',smallest_index)

if smallest_index < 600:
    if smallest_index == speedy_index:
        length = speedy_index -1
    else:
        length = hybrid_index #-1 
else:
    length = smallest_index
print('the smallest length now is:',length)
length_2calc = length
...
cyclic_data, cyclic_lons = add_cyclic_point(averaged_error_4, coord=lon)
...
ax2.coastlines()
cyclic_data, cyclic_lons = add_cyclic_point(averaged_error_speedy_4, coord=lon)
lons2d,lats2d = np.meshgrid(cyclic_lons,lat)

cf = ax2.contourf(lons2d, lats2d,cyclic_data,levels=np.arange(0,5.1,.005),extend='both')
plt.colorbar(cf,label='(K)',fraction=0.046, pad=0.04)
ax2.set_title('Previous Hybrid (1.3) LETKF Analysis Error \n Low Level Temperature')
...
ax3.coastlines()
ax3.set_title('Difference (New Hybrid - Previous Hybrid)')
