...
# Use shortest time scale from the three analysis files
# else the index is out of range.

hybrid_length = temp_500_analysis.shape[0]
speedy_length = temp_500_analysis_speedy.shape[0]

smallest_length = min(hybrid_length,speedy_length)
print('smallest length before delete last timestep is',smallest_length)
if smallest_length < 600: 
    if smallest_length == hybrid_length:
        length = hybrid_length
    else:
        length = speedy_length - 1
else:
    length = smallest_length

#check that length is minus 1 for old hybrid which is actually "speedy" here..
#because waste of time to change all the names

print('the new adjusted length is:',length)
...
ax1.set_title('Newly Trained\nHybrid LETKF (1.3) Analysis Error\nLow Level Specific Humidity')
...
ax3.coastlines()
ax3.set_title('Difference (New Hybrid - SPEEDY)')
