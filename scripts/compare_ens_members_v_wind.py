...
plt.colorbar(cf,label='(m/s)',fraction=0.046, pad=0.04)
plt.tight_layout()
plt.savefig('wind/MAP_OF_V_WIND.pdf',dpi=1200)
plt.savefig('wind/MAP_OF_V_WIND.jpeg',dpi=1200)
#plt.show()
...
plt.plot(date_list,analysis_rmse_speedy,color='k',linewidth=.75,label='RMSE New SPEEDY')
...
plt.ylim(3,6)
plt.tight_layout()
plt.savefig('wind/PLOT_OF_V_WIND.pdf',dpi=1200)
plt.savefig('wind/PLOT_OF_V_WIND.jpeg',dpi=1200)
#plt.show()
...
lons2d, lats2d = np.meshgrid(lon,lat)
'''3rd calculation for 40 member covariance inflation 1.5'''
@jit()
def rms(true,prediction):
    return np.sqrt(np.nanmean((prediction-true)**2))
