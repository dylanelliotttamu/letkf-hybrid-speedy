...
start_year = 1997 #2000

startdate = datetime(1997,1,1,0)  #datetime(2000,1,1,0)
enddate = datetime(2010,12,30,0) #datetime(2001,1,5,0)

nature_file = f'/skydata2/troyarcomano/ERA_5/{start_year}/era_5_y{start_year}_regridded_mpi_fixed_var.nc'
analysis_file_speedy = '/skydata2/troyarcomano/letkf-hybrid-speedy/DATA/era5_letkf_1990_2011/era5_letkf_1990_2011.nc'
#analysis_file = '/scratch/user/troyarcomano/letkf-hybrid-speedy/experiments/first_test_letkf_hybrid/hybrid/anal_mean.nc'
##   analysis_file = '/skydata2/troyarcomano/letkf-hybrid-speedy/DATA/ensemble/anal/mean/test.nc'
#analysis_file = '/scratch/user/troyarcomano/letkf-hybrid-speedy/experiments/conv_inf1.5/hybrid/anal_mean.nc'
#spread_file = '/skydata2/troyarcomano/letkf-hybrid-speedy/experiments/first_test_letkf_hybrid/hybrid/anal_sprd.nc'
## spread_file = '/skydata2/troyarcomano/letkf-hybrid-speedy/DATA/ensemble/anal/mean/test.nc'
#spread_file = '/scratch/user/troyarcomano/letkf-hybrid-speedy/experiments/conv_inf1.5/hybrid/anal_sprd.nc'

analysis_file = '~/letkf-speedy-rtpp/speedy30rtpp08.nc' #'~/stable_run/rtpp_0_3.nc' #'/skydata2/dylanelliott/letkf-hybrid-speedy/DATA/ensemble/anal/mean/test.nc'
spread_file = '~/letkf-speedy-rtpp/speedy30rtpp08.nc' #'~/stable_run/rtpp_0_3.nc' # '/skydata2/troyarcomano/letkf-hybrid-speedy/DATA/ensemble/anal/mean/test.nc'
...
xgrid = 96
ygrid = 48
length = 1400 #338  #1450 ##338 #160#64#177#1400#455
...
colored plot that shows LETKF mean analysis error from era5 data

'''
cf = ax1.contourf(lons2d, lats2d,averaged_error*1000,levels=np.arange(0,3.1,0.1),extend='both')
plt.colorbar(cf,shrink=0.6)
plt.title('LETKF RTPP_0.8 Analysis RMSE by Grid Point\n Low Level Specific Humidity (1997)')
plt.show()
...
plot of difference of lektf-speedy-rtpp minus letkf-speedy-cov_infl 

'''
ax1 = plt.subplot(111,projection=ccrs.PlateCarree())
ax1.coastlines()

cf = ax1.contourf(lons2d, lats2d,cyclic_data,levels=np.arange(-5,5.1,1),extend='both',cmap='seismic')
plt.title('LETKF Analysis RMSE Difference of RTPP_0.8 to Multi_Cov_Infl\n for Low Level Specific Humidity (1997)\n')
plt.colorbar(cf,shrink=0.6)
plt.show()
...
base = datetime(1997,1,1,0)#datetime(2000,1,1,0)

date_list = [base + timedelta(days=x/4) for x in range(length)]
plt.plot(date_list,analysis_rmse,color='r',label='RMSE Speedy RTPP=0.8')
plt.plot(date_list,analysis_rmse_speedy,color='b',label='RMSE SPEEDY Cov_infl = 1.3')
plt.axhline(y=np.average(analysis_rmse[20::]), color='r', linestyle='--',label="Average RMSE Speedy RTPP=0.8")
plt.axhline(y=np.average(analysis_rmse_speedy[20::]), color='b', linestyle='--',label="Average RMSE SPEEDY Cov_infl = 1.3")
...
plt.title('LETKF Analysis RMSE averaged over all lat/lon\n 1982 Low Level Specific Humidity')
...
#plt.xlim([datetime(2000, 1, 1,0), datetime(2000, 3, 1,0)])
plt.xlim([datetime(1997, 1, 1,0), datetime(1998, 1, 5,0)])
plt.show()
