...
#40member _____ analysis_file_speedy = '/skydata2/dylanelliott/letkf-hybrid-speedy/DATA/uniform_letkf_anal/uniform_analysis_2011_01_to_2012_05.nc'
# USED PREVIOUSLYanalysis_file_speedy = '/skydata2/dylanelliott/letkf-hybrid-speedy/DATA/uniform_letkf_anal/uniform_analysis_81_12.nc'
#40member _____ analysis_file = '/skydata2/dylanelliott/letkf-hybrid-speedy/DATA/uniform_letkf_anal/40member_hybrid_20110101_2011052906/anal/mean/mean.nc'
#analysis_file = '/skydata2/dylanelliott/letkf-hybrid-speedy/DATA/uniform_letkf_anal/uniform_analysis_hybrid_2011_01_to_05_blowup_cov13.nc' #onlygoesuntilMay1
# USED PREVIOUSLY analysis_file = '/skydata2/dylanelliott/letkf-hybrid-speedy/DATA/uniform_letkf_anal/uniform_hybrid_anal_Jan1_May1_2011.nc'
...
xgrid = 96
ygrid = 48
length = 592#240  #1450 ##338 #160#64#177#1400#455
...
analysis_rmse_west_africa = np.zeros((length))
analysis_rmse_speedy_west_africa = np.zeros((length))

analysis_error_west_africa = np.zeros((length))
analysis_error_speedy_west_africa  = np.zeros((length))
...
for i in range(length):
    ...
    analysis_rmse_west_africa[i] = rms(temp_500_nature[i*6,y_WA,x_WA],temp_500_analysis[i,y_WA,x_WA])
    analysis_rmse_speedy_west_africa[i] = rms(temp_500_nature[i*6,y_WA,x_WA],temp_500_analysis_speedy[i,y_WA,x_WA])
    analysis_error_west_africa[i] = temp_500_analysis[i,y_WA,x_WA] - temp_500_nature[i*6,y_WA,x_WA] 
    analysis_error_speedy_west_africa[i] = temp_500_analysis_speedy[i,y_WA,x_WA] - temp_500_nature[i*6,y_WA,x_WA] 
...
# avg error
averaged_error_west_africa = np.average(abs(analysis_error_west_africa[24::]),axis=0)
averaged_error_speedy_west_africa = np.average(abs(analysis_error_speedy_west_africa[24::]),axis=0)
...
plt.plot(date_list,analysis_rmse_west_africa,color='r',label='RMSE Hybrid ') #cov-infl1.3
plt.plot(date_list,analysis_rmse_speedy_west_africa,color='b',label='RMSE SPEEDY')
plt.axhline(y=np.average(analysis_rmse_west_africa[20::]), color='r', linestyle='--',label="Average RMSE Hybrid")
plt.axhline(y=np.average(analysis_rmse_speedy_west_africa[20::]), color='b', linestyle='--',label="Average RMSE SPEEDY")
print('average rmse Hybrid', np.average(analysis_rmse_west_africa[20::]))
print('average rmse SPEEDY', np.average(analysis_rmse_speedy_west_africa[20::]))
