...
cf = ax1.contourf(lons2d, lats2d,cyclic_data,levels=np.arange(0,10,.01),extend='both')
...
cf = ax2.contourf(lons2d, lats2d,cyclic_data,levels=np.arange(0,10,.01),extend='both')
plt.colorbar(cf,label='(m/s)',fraction=0.046, pad=0.04)
...
cf = ax3.contourf(lons2d, lats2d,cyclic_data,levels=np.arange(-5,5,.01),extend='both',cmap='seismic')
plt.colorbar(cf,label='(m/s)',fraction=0.046, pad=0.04)
#plt.tight_layout()
#plt.savefig('SPARSE_network_map_200hPa_V_Wind.jpeg',dpi=1200)
plt.show()
...
hybrid_average = np.average(analysis_rmse[20::])
speedy_average = np.average(analysis_rmse_speedy[20::])
percent_change = ((hybrid_average - speedy_average) / speedy_average ) * 100 
