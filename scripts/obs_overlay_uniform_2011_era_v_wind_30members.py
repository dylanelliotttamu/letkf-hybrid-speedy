...
xgrid = 96
ygrid = 48
length = 1457# 240 for 3 months  #1450 ##338 #160#64#177#1400#455
...
for i in range(np.shape(network_ij)[0]):
    plt.scatter(lon[network_ij[i][0]],lat[network_ij[i][1]],s=1,color='k')
...
plt.colorbar(cf,label='(m/s)')
ax2.set_title('30 MEMBER ENSEMBLE\nSPEEDY LETKF Analysis Error \n 200 hPa Meridional Wind')
...
cf = ax3.contourf(lons2d, lats2d,cyclic_data,levels=np.arange(-5,5.1,.05),extend='both',cmap='seismic')
for i in range(np.shape(network_ij)[0]):
    plt.scatter(lon[network_ij[i][0]],lat[network_ij[i][1]],s=1,color='k')
...
plt.axhline(y=np.average(analysis_rmse[20::]), color='r', linestyle='--',linewidth=.75,label="Average RMSE Hybrid")
plt.axhline(y=np.average(analysis_rmse_speedy[20::]), color='b', linestyle='--',linewidth=.75,label="Average RMSE SPEEDY")
...
plt.xlim([datetime(2011, 1, 1,0), datetime(2012, 2, 1,0)])
plt.ylim(3,10)
plt.show()
