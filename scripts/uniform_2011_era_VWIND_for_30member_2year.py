...
xgrid = 96
ygrid = 48
length = 1457#240  #1450 ##338 #160#64#177#1400#455
...
#plt.colorbar(cf,label='(g/kg)')
#ax2.set_title('40 MEMBER ENSEMBLE\nSPEEDY LETKF Analysis Error \n Low Level Specific Humidity')
...
averaged_error_2 = np.average(abs(analysis_error[24::,:,:]),axis=0)
averaged_error_speedy_2 = np.average(abs(analysis_error_speedy[24::,:,:]),axis=0)
...
plt.plot(date_list,analysis_rmse,color='r',linewidth=.75,label='RMSE Hybrid 30 mem') #cov-infl1.3
plt.plot(date_list,analysis_rmse_speedy,color='b',linewidth=.75,label='RMSE SPEEDY 40 mem')
plt.axhline(y=np.average(analysis_rmse[20::]), color='r', linestyle='--',label="Average RMSE Hybrid 30 mem")
plt.axhline(y=np.average(analysis_rmse_speedy[20::]), color='b', linestyle='--',label="Average RMSE SPEEDY 40 mem")

plt.plot(date_list,analysis_rmse_speedy_2,color='g',linewidth=.75,label='RMSE SPEEDY 20 mem')
plt.axhline(y=np.average(analysis_rmse_speedy_2[20::]), color='g', linestyle='--',label="Average RMSE SPEEDY 20 mem")
...
plt.title('30 Member Hybrid & 40 Member SPEEDY & 20 Member SPEEDY \nLETKF Analysis Error\n 200 hPa Meridional Wind')
...
plt.xlim([datetime(2011, 1, 1,0), datetime(2011, 12, 31,18)])
