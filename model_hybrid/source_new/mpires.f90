   ...
   contains
     ...
     subroutine distribute_ic_and_speedy_forecast(res,wholegrid4d,wholegrid2d,forecast_4d,forecast_2d,sender)
        ...
        if(sender) then
           ...
           do proc_num=0,mpi_res%numprocs
              if(proc_num .not. mpi_res%proc_num) then
                call processor_decomposition_manual(proc_num,mpi_res%numprocs,res%model_parameters%number_of_regions,region_indices)
                ...
                do i=1, number_of_regions
                   do j=1, res%model_parameters%num_vert_levels
                      ...
                      !print *, 'sending from root to',proc_num,'region
        ...
        if(.not. sender) then
          ...
          do i=1, res%model_parameters%num_of_regions_on_proc
             do j=1, res%model_parameters%num_vert_levels
                ...
                !print *, res%model_parameters%irank,'receiving feedback from
                !root for region,
                !level',res%model_parameters%region_indices(i),j,res%reservoir(i,j)%feedback(res%grid(i,j)%precip_start:res%grid(i,j)%precip_end)
                ...
                if(.not. res%model_parameters%ml_only) then
                  ...
                  call MPI_RECV(sendreceivedata,res%reservoir(i,j)%chunk_size_speedy,MPI_DOUBLE_PRECISION,from,tag,mpi_res%mpi_world,MPI_STATUS_IGNORE,mpi_res%ierr)
