program main
  ...
  left_over = mod(mpi_res%numprocs,num_ensembles)
  ...
  if(.not. allocated(forecast_grid4d))  allocate(forecast_grid2d(ens_batches,xgrid,ygrid))

  !Run all of the ensemble SPEEDY components of forecasts in parallel
  mem_counter = mem_counter + 1
  do ensemble_mem=1, num_ensembles
     if(mod(ensemble_mem,num_ensembles) == mpires%irank)
  ...
  mem_counter = 1
  do ensemble_mem=1, ensemble_members

     if(mod(ensemble_mem,num_ensembles) == mpires%irank)
       call distribute_ic_and_speedy_forecast(res,grid4d(mem_counter,:,:,:,:),grid2d(mem_counter,:,:),forecast_grid4d(mem_counter,:,:,:,:),forecast_grid2d(mem_counter,:,:),.True.)
     else
