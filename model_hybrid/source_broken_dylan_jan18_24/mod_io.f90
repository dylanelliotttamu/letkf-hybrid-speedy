module mod_io
   ...
   contains 
     ...
     subroutine write_netcdf_1d_non_met_data_real_overwrite(var,varname,filename,units,x_dim,overwrite)
       real(kind=dp), intent(in)    :: var(:)
       ...
       if(.not.file_exists(filename)) then
          call nc_check(nf90_create(path=filename,cmode=NF90_CLOBBER,ncid=file_id),message='write_netcdf_1d_non_met_data_real_over nf90_create')
       else
          call nc_check(nf90_open(filename,nf90_write,file_id),message='write_netcdf_1d_non_met_data_real_over nf90_open')
       endif

       if(overwrite) then
         call nc_check(nf90_inq_dimid(file_id, x_dim,xdim_id),message='write_netcdf_1d_non_met_data_real_over nf90_inq_dimid')
       else
         call nc_check(nf90_def_dim(file_id, x_dim, dimsx, xdim_id),message='write_netcdf_1d_non_met_data_real_over nf90_def_dim')
       endif
       ...
       if(overwrite) then
         ...
         call nc_check(nf90_def_var(file_id,varname,NF90_REAL,arrdims,array_id),message='write_netcdf_1d_non_met_data_real_over nf90_def_var')

         call nc_check(nf90_put_att(file_id, array_id, "units", units),message='write_netcdf_1d_non_met_data_real_over nf90_put_att')

         call nc_check(nf90_enddef(file_id),message='write_netcdf_1d_non_met_data_real_over nf90_enddef') 
       endif 

       !Write out the values
       call nc_check(nf90_put_var(file_id, array_id, var,start=start, count=varcount),message='write_netcdf_1d_non_met_data_real_over nf90_put_var')

       call nc_check(nf90_close(file_id),message='write_netcdf_1d_non_met_data_real_over nf90_close')
     end subroutine
   ...
   subroutine read_full_file_4d(grid4d,grid2d,timestep,filename)
    ...
    subroutine read_trained_res(reservoir,model_parameters,grid)
      ...
      character(len=2) :: ensemble_num_char

      integer :: ncid

      file_path='/skydata2/dylanelliott/ML_SPEEDY_WEIGHTS_uniform_29yr_training_cov_1_5_speedy_iterative1_offbyonebugfix/'
      ...
      write(ensemble_num_char,'(i0.2)') model_parameters%ensemble_member
      ...
      call read_netcdf_1d_dp_opened('res_state'//ensemble_num_char,ncid,reservoir%saved_state)
    ...
    subroutine read_res_state(reservoir,model_parameters,grid)
      use mod_utilities, only : reservoir_type,model_parameters_type,grid_type
    ...
    subroutine read_trained_ocean_res(reservoir,model_parameters,grid)
      ...
      !file_path='/skydata2/dylanelliott/ML_SPEEDY_WEIGHTS_era5trainedweights/'
      write(worker_char,'(i0.4)') reservoir%assigned_region
      print *, 'reservoir%assigned_region',reservoir%assigned_region
