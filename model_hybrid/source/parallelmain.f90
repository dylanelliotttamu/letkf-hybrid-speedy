program main
  ...
  integer :: left_over, mem_counter, ensembles_per_proc, ens_batches, ens_batch
  integer :: ensembles_per_batch
  ...
  ens_batches = num_ensembles/mpi_res%numprocs
  left_over = mod(num_ensembles,mpi_res%numprocs)

  print *, 'ens_batches,left_over',ens_batches,left_over
  print *, 'mpi_res%numprocs,num_ensembles,mod(mpi_res%numprocs,num_ensembles)',mpi_res%numprocs,num_ensembles,mod(num_ensembles,mpi_res%numprocs)
  if(left_over > 0) then 
    ens_batches = ens_batches + 1
  endif 
   
  !Need to make this for both the ICs and the SPEEDY forecast
  if(.not. allocated(grid4d))           allocate(grid4d(ens_batches,res%model_parameters%full_predictvars,xgrid,ygrid,zgrid))
  if(.not. allocated(grid2d))           allocate(grid2d(ens_batches,xgrid,ygrid))
  if(.not. allocated(forecast_grid4d))  allocate(forecast_grid4d(ens_batches,res%model_parameters%full_predictvars,xgrid,ygrid,zgrid))
  if(.not. allocated(forecast_grid2d))  allocate(forecast_grid2d(ens_batches,xgrid,ygrid))
  !Run all of the ensemble SPEEDY components of forecasts in parallel
  mem_counter = 1!mem_counter + 1
  do ensemble_mem=1, num_ensembles
     print *, 'ensemble_mem, mpi_res%proc_num,mod(ensemble_mem,mpi_res%numprocs),ens_batches',ensemble_mem, mpi_res%proc_num,mod(ensemble_mem,mpi_res%numprocs),mpi_res%numprocs
     if(mod(ensemble_mem,mpi_res%numprocs) == mpi_res%proc_num) then 
        write(ensemble_path,'(i0.3)') ensemble_mem

        ensemble_path_full = trim(root_path)//'/'//ensemble_path
        print *, 'ensemble_path_full forecasts',ensemble_path_full, mpi_res%proc_num
        ...
        print *, 'current path forecasts', trim(current_path), mpi_res%proc_num
        ...
        call agcm_main()
        ! grid4d(ens_batches,res%model_parameters%full_predictvars,xgrid,ygrid,zgrid)
        grid4d(mem_counter,1,:,:,:) = reshape(tg1_ic,[xgrid,ygrid,zgrid])
        grid4d(mem_counter,2,:,:,:) = reshape(ug1_ic,[xgrid,ygrid,zgrid])
        grid4d(mem_counter,3,:,:,:) = reshape(vg1_ic,[xgrid,ygrid,zgrid])
        grid4d(mem_counter,4,:,:,:) = reshape(qg1_ic,[xgrid,ygrid,zgrid])
        ! DYLAN 
        ! print statment here 
        !print *, 'MEM1 print grid4d', grid4d(1,4,1,1,1)
        !print *,  'MEM2 print grid4d', grid4d(2,4,1,1,1)
        !prin
        
        grid2d(mem_counter,:,:) = reshape(pslg1_ic,[xgrid,ygrid])

        forecast_grid4d(mem_counter,1,:,:,:) = reshape(tg1,[xgrid,ygrid,zgrid])
        forecast_grid4d(mem_counter,2,:,:,:) = reshape(ug1,[xgrid,ygrid,zgrid])
        forecast_grid4d(mem_counter,3,:,:,:) = reshape(vg1,[xgrid,ygrid,zgrid])
        forecast_grid4d(mem_counter,4,:,:,:) = reshape(qg1,[xgrid,ygrid,zgrid])
        print *, 'mem_counter', mem_counter 
        if(ensemble_mem .eq. 1) then
            print *, 'DYLAN grid4d MEM1 x=54,y=28' 
            print *, grid4d(1,4,54,28,:)
            print *, 'forecast_grid4d MEM1  x=54,y=28'
            print *, forecast_grid4d(1,4,54,28,:)
        endif
        if(ensemble_mem .eq. 2) then
            print *, 'DYLAN grid4d MEM2 x=54,y=28' 
            print *, grid4d(1,4,54,28,:)
            print *, 'forecast_grid4d MEM2 x=54,y=28'
            print *, forecast_grid4d(1,4,54,28,:)
        endif

        forecast_grid2d(mem_counter,:,:) = reshape(pslg1,[xgrid,ygrid])
  ...
  do ensemble_mem=1, num_ensembles
     ...
     ensemble_path_full = trim(root_path)//'/'//ensemble_path
     print *, 'ensemble_path_full hybrid stuff',ensemble_path_full, mpi_res%proc_num
     ...
     print *, 'current path hybrid stuff', trim(current_path), mpi_res%proc_num
     print *, 'ensemble_mem, proc_num, grid4d(mem_counter,1,:,:,:)',mem_counter,ensemble_mem,mpi_res%proc_num,grid4d(mem_counter,1,20,20,:)
     print *, 'ensemble_mem, proc_num, forecast_grid4d(mem_counter,1,:,:,:)',mem_counter,ensemble_mem,mpi_res%proc_num,forecast_grid4d(mem_counter,1,20,20,:)
     if(mod(ensemble_mem,mpi_res%numprocs) == mpi_res%proc_num) then
       print *, 'ensemble_mem, proc_num is the sender',ensemble_mem, mpi_res%proc_num
       !call write_netcdf_speedy_full(grid4d(mem_counter,:,:,:,:),grid2d(mem_counter,:,:),1,'ics.nc')
       !call write_netcdf_speedy_full(forecast_grid4d(mem_counter,:,:,:,:),forecast_grid2d(mem_counter,:,:),1,'speedy_forecast.nc')
       call distribute_ic_and_speedy_forecast(res,grid4d(mem_counter,:,:,:,:),grid2d(mem_counter,:,:),forecast_grid4d(mem_counter,:,:,:,:),forecast_grid2d(mem_counter,:,:),sender=.True.)
       mem_counter = mem_counter + 1
     else
       call distribute_ic_and_speedy_forecast(res,sender=.False.)
     endif
