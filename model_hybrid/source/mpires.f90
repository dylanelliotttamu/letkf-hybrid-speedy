module mpires
   ...
   use mod_tsteps, only : sixhrrun, ipout, ihout
   use mod_physvar, only : ugr_h, vgr_h, tgr_h, qgr_h, psgr_h
   ...
   contains
     ...
     subroutine sendrecievegrid(res,timestep,ocean_model)
        ...
        if(mpi_res%is_root) then
           ...
           !NOTE TODO THIS IS ONLY FOR ERA5 based model trained upside down 
           !wholegrid4d = wholegrid4d(:,:,:,8:1:-1)
           print *, 'wholegrid4d assembled',minval(wholegrid4d(1,:,:,:)), maxval(wholegrid4d(1,:,:,:))
           ...
           call write_netcdf_speedy_full(wholegrid4d,wholegrid2d,1,'hybrid_predict.nc')

           call getcwd(current_path)

           if (ihout) then
            print *, 'writing out hybrid forecasts to using iogrid(6)', trim(current_path)
            !if(ipout) print *, 'calling iogrid (2)', trim(current_path)
            !if (ipout) call iogrid (8) !output for every 6 hours
            call iogrid (6) !gridded data output for every 6 hours
           end if

           if (sixhrrun) then
              call restart (2)
              print *,'mpires restart(2)'
           end if
        endif
     ...
     !NOTE TODO change so that not just root can send stuff 
     subroutine distribute_ic_and_speedy_forecast(res,wholegrid4d,wholegrid2d,forecast_4d,forecast_2d,sender)
        use resdomain, only : tile_4d_and_logp_to_local_state_input, tile_4d_and_logp_state_vec_res1d, unstandardize_state_vec_input, tile_4d_and_logp_full_grid_to_local_res_vec, &
                              processor_decomposition_manual, getxyresextent, tile_full_grid_with_local_state_vec_res1d, standardize_state_vec_input, standardize_state_vec_res, &
                              tile_4d_to_local_state_input, tile_full_2d_grid_with_local_res, tile_4d_and_logp_to_local_state_input_slab
        ...
        real(kind=dp), intent(in), optional :: wholegrid4d(:,:,:,:), wholegrid2d(:,:), forecast_4d(:,:,:,:), forecast_2d(:,:)

        real(kind=dp), allocatable :: sendreceivedata(:), temp4d(:,:,:,:), temp2d(:,:), temp3d(:,:,:), temp1d(:)

        logical, optional :: sender 
        ...
        if(sender) then
           do i=1, res%model_parameters%num_of_regions_on_proc
              do j=1, res%model_parameters%num_vert_levels
                 ...
                 call tile_4d_and_logp_to_local_state_input(res%model_parameters,res%model_parameters%region_indices(i),j,wholegrid4d,wholegrid2d,temp2d,res%reservoir(i,j)%feedback(1:res%reservoir(i,j)%reservoir_numinputs-res%reservoir(i,j)%tisr_size_input-res%reservoir(i,j)%sst_size_input))
           ...
           do proc_num=0,mpi_res%numprocs-1
              if(proc_num /= mpi_res%proc_num) then
                call processor_decomposition_manual(proc_num,mpi_res%numprocs,res%model_parameters%number_of_regions,region_indices)
                ...
                do i=1, number_of_regions
                   do j=1, res%model_parameters%num_vert_levels
                      ...
                      !num',region_indices(i),receive_size

                      allocate(sendreceivedata(receive_size))

                      call tile_4d_and_logp_to_local_state_input(res%model_parameters,region_indices(i),j,wholegrid4d,wholegrid2d,temp2d,sendreceivedata)

                      to = proc_num

                      tag = counter
    
                      !print *, 'sending from',mpi_res%proc_num,'to',proc_num,'region', i, 'sendreceivedata',sendreceivedata
        ...
        print *, 'here',mpi_res%proc_num,sender
        ...
        if(.not. sender) then
          ...
          do i=1, res%model_parameters%num_of_regions_on_proc
             do j=1, res%model_parameters%num_vert_levels
                ...
                call MPI_RECV(sendreceivedata,receive_size,MPI_DOUBLE_PRECISION,MPI_ANY_SOURCE,tag,mpi_res%mpi_world,MPI_STATUS_IGNORE,mpi_res%ierr)

                !print *, 'receiving from',MPI_ANY_SOURCE,'tag',tag,'worker',mpi_res%proc_num, 'region', i, 'sendreceivedata', sendreceivedata
                ...
                if(.not. res%model_parameters%ml_only) then
                  ...
                  call MPI_RECV(sendreceivedata,res%reservoir(i,j)%chunk_size_speedy,MPI_DOUBLE_PRECISION,MPI_ANY_SOURCE,tag,mpi_res%mpi_world,MPI_STATUS_IGNORE,mpi_res%ierr)
!prediction,MPI_DOUBLE_PRECISION,from,tag,mpi_res%mpi_world,MPI_STATUS_IGNORE,mpi_res%ierr)
        ...
        do i=1, res%model_parameters%num_of_regions_on_proc
           do j=1, res%model_parameters%num_vert_levels
              if(res%reservoir(i,j)%tisr_input_bool) then
                 call get_tisr_by_date(res%reservoir(i,j),res%grid(i,j),res%model_parameters,-1,res%reservoir(i,j)%feedback(res%grid(i,j)%tisr_start:res%grid(i,j)%tisr_end))
     ...
     subroutine distribute_speedy_forecast(res,grid4d,grid2d)
        use resdomain, only : tile_4d_and_logp_to_local_state_input, tile_4d_and_logp_state_vec_res1d, unstandardize_state_vec_input, tile_4d_and_logp_full_grid_to_local_res_vec, &
                              processor_decomposition_manual, getxyresextent, tile_full_grid_with_local_state_vec_res1d, standardize_state_vec_input, standardize_state_vec_res, &
                              tile_4d_to_local_state_input, tile_full_2d_grid_with_local_res, tile_4d_and_logp_to_local_state_input_slab
        ...
        real(kind=dp), intent(in), optional :: grid4d(:,:,:,:), grid2d(:,:) 
     ...
     subroutine write_truth_data(res,timestep)
       ...
       if(mpi_res%is_root) then
           ...
           file_end = '.nc'
           trial_word = 'trial_'
           file_path = '/skydata2/troyarcomano/Predictions/Hybrid/'
